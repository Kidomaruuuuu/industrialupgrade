...
public abstract class TileEntityInventory extends TileEntityBlock implements ISidedInventory,
    ...
    private final List<InfoInvSlots> infoInvSlotsList = new ArrayList<>();
    ...
    protected static int calcRedstoneFromInvSlots(Iterable<InvSlot> invSlots) {
        int space = 0;
        int used = 0;
        Iterator var3 = invSlots.iterator();
    ...
    public int getBaseIndex(InvSlot invSlot) {
        ...
        InvSlot slot;
        for (Iterator var3 = this.invSlots.iterator(); var3.hasNext(); ret += slot.size()) {
            slot = (InvSlot) var3.next();
            if (slot == invSlot) {
                return ret;
            }
    ...
    public InvSlot getInventorySlot(String name) {
        Iterator var2 = this.invSlots.iterator();
