...
public class TileEntityQCable extends TileEntityBlock implements IQEConductor, INetworkTileEntityEventListener {
    ...
    private volatile TileEntityQCable.CableRenderState renderState;
    private IWorldTickCallback continuousUpdate;

    public TileEntityQCable(QEType cableType, int insulation) {
        this();
    ...
    public Ic2BlockStateInstance getExtendedState(Ic2BlockStateInstance state) {
        ...
        TileEntityQCable.CableRenderState cableRenderState = this.renderState;
                ...
                if (foam == CableFoam.Soft) {
                    this.continuousUpdate = world1 -> {
                        if (world1.rand.nextFloat() < BlockFoam.getHardenChance(
                                world1,
                                TileEntityQCable.this.pos,
                                TileEntityQCable.this.getBlockType().getState(TeBlock.cable),
                                FoamType.normal
                        )) {
                            TileEntityQCable.this.changeFoam(CableFoam.Hardened, false);
                        }
    ...
    private void updateRenderState() {
        this.renderState = new TileEntityQCable.CableRenderState(
                this.cableType,
                this.insulation,
                this.foam,
                this.connectivity,
    ...
    public static class CableRenderState {
        ...
        public final QEType type;
        public final int insulation;
        public final CableFoam foam;
        public final int connectivity;
        public final boolean active;

        public CableRenderState(QEType type, int insulation, CableFoam foam, int connectivity, boolean active) {
            this.type = type;
            this.insulation = insulation;
            this.foam = foam;
        ...
        public boolean equals(Object obj) {
            if (obj == this) {
                return true;
            } else if (!(obj instanceof TileEntityQCable.CableRenderState)) {
                return false;
            } else {
                TileEntityQCable.CableRenderState o = (TileEntityQCable.CableRenderState) obj;
