...
public class TileEntityHeatPipes extends TileEntityBlock implements IHeatConductor, INetworkTileEntityEventListener {

    public static final float insulationThickness = 0.0625F;
    public static final IUnlistedProperty<TileEntityHeatPipes.CableRenderState> renderStateProperty = new UnlistedProperty(
            "renderstate",
            TileEntityHeatPipes.CableRenderState.class
    );
    ...
    private volatile TileEntityHeatPipes.CableRenderState renderState;
    private IWorldTickCallback continuousUpdate;

    public TileEntityHeatPipes(HeatType cableType, int insulation) {
        this();
    ...
    public Ic2BlockStateInstance getExtendedState(Ic2BlockStateInstance state) {
        ...
        TileEntityHeatPipes.CableRenderState cableRenderState = this.renderState;
    ...
    public boolean canInteractWith(IHeatTile tile, EnumFacing side) {
                ...
                if (foam == CableFoam.Soft) {
                    this.continuousUpdate = world1 -> {
                        if (world1.rand.nextFloat() < BlockFoam.getHardenChance(
                                world1,
                                TileEntityHeatPipes.this.pos,
                                TileEntityHeatPipes.this.getBlockType().getState(TeBlock.cable),
                                FoamType.normal
                        )) {
                            TileEntityHeatPipes.this.changeFoam(CableFoam.Hardened, false);
                        }
    ...
    private void updateRenderState() {
        this.renderState = new TileEntityHeatPipes.CableRenderState(
                this.cableType,
                this.insulation,
                this.foam,
                this.connectivity,
    ...
    public static class CableRenderState {
        ...
        public final HeatType type;
        public final int insulation;
        public final CableFoam foam;
        public final int connectivity;
        public final boolean active;

        public CableRenderState(HeatType type, int insulation, CableFoam foam, int connectivity, boolean active) {
            this.type = type;
            this.insulation = insulation;
            this.foam = foam;
        ...
        public boolean equals(Object obj) {
            if (obj == this) {
                return true;
            } else if (!(obj instanceof TileEntityHeatPipes.CableRenderState)) {
                return false;
            } else {
                TileEntityHeatPipes.CableRenderState o = (TileEntityHeatPipes.CableRenderState) obj;
