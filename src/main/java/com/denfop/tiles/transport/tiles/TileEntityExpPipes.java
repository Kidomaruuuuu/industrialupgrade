...
public class TileEntityExpPipes extends TileEntityBlock implements IEXPConductor, INetworkTileEntityEventListener {
    ...
    public static final IUnlistedProperty<TileEntityExpPipes.CableRenderState> renderStateProperty = new UnlistedProperty<>(
            "renderstate",
            TileEntityExpPipes.CableRenderState.class
    );
    ...
    private volatile TileEntityExpPipes.CableRenderState renderState;
    private IWorldTickCallback continuousUpdate;

    public TileEntityExpPipes(ExpType cableType, int insulation) {
        this();
    ...
    public Ic2BlockStateInstance getExtendedState(Ic2BlockStateInstance state) {
        ...
        TileEntityExpPipes.CableRenderState cableRenderState = this.renderState;
                ...
                if (foam == CableFoam.Soft) {
                    this.continuousUpdate = world1 -> {
                        if (world1.rand.nextFloat() < BlockFoam.getHardenChance(
                                world1,
                                TileEntityExpPipes.this.pos,
                                TileEntityExpPipes.this.getBlockType().getState(TeBlock.cable),
                                FoamType.normal
                        )) {
                            TileEntityExpPipes.this.changeFoam(CableFoam.Hardened, false);
                        }
    ...
    private void updateRenderState() {
        this.renderState = new TileEntityExpPipes.CableRenderState(
                this.cableType,
                this.insulation,
                this.foam,
                this.connectivity,
    ...
    public static class CableRenderState {
        ...
        public CableRenderState(ExpType type, int insulation, CableFoam foam, int connectivity, boolean active) {
            this.type = type;
            this.insulation = insulation;
            this.foam = foam;
        ...
        public boolean equals(Object obj) {
            if (obj == this) {
                return true;
            } else if (!(obj instanceof TileEntityExpPipes.CableRenderState)) {
                return false;
            } else {
                TileEntityExpPipes.CableRenderState o = (TileEntityExpPipes.CableRenderState) obj;
