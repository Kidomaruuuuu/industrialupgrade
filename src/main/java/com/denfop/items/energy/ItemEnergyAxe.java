...
public class ItemEnergyAxe extends ItemTool implements IElectricItem, IUpgradeWithBlackList, IModelRegister {
    ...
    public int getHarvestLevel(
        ...
        return (!toolClass.equals("axe")) ? super.getHarvestLevel(stack, toolClass, player, blockState)
    ...
    public boolean canHarvestBlock(@Nonnull final IBlockState state, @Nonnull final ItemStack stack) {
        return  (Items.DIAMOND_AXE.canHarvestBlock(state, stack)
                || Items.DIAMOND_AXE.getDestroySpeed(stack, state) > 1.0F || mineableBlocks.contains(state));
