package aroma1997.uncomplication.enet.transformer;
...
public class EnergyNetLocal {
        ...
        if (amount > 0.0D) {
            for (EnergyPath energyPath : energyPaths) {
                ...
                if (energyProvided > energyLoss) {
                    ...
                    double adding = Math.min(providing, demandedEnergy);
                    if (adding <= 0.0D) {
                        continue;
                    }
    ...
    private boolean conductorToWeak(List<IEnergyConductor> par1, double energyToSend) {
        boolean flag = false;
                ...
                for (IEnergySource entry : this.sources) {
                    ...
                    if (entry != null) {
                        ...
                        if(entry instanceof TileEntityNuclearReactorElectric || entry instanceof TileEntityBaseNuclearReactorElectric)
                            offered /=2;
                        double offer = Math.min(offered, EnergyNet.instance
                                .getPowerFromTier(entry.getSourceTier()));
                        if (offer > 0.0D) {
                            for (int i = 0; i < getPacketAmount(entry); i++) {

                                offer = Math.min(offered, EnergyNet.instance
                                        .getPowerFromTier(entry.getSourceTier()));
                                if (offer < 1) {
                                    break;
                                }
